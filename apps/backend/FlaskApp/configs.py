...
class DevConfigs:
  ...
  postgres_manager= PostgresManager(ENV_VARS.get("SQLALCHEMY_POSTGRESSQL_0_CONN_STRING"))
  ...
  # square_manager = SquareManager(ENV_VARS.get("SQUARE_SANDBOX_ACCESS_TOKEN_0"))
  firebase_manager = FirebaseManager(ENV_VARS)

  def _create_app_info_obj(self,backend_port=5016):

    return {
      'access_control_allow_origin':['https://example.com:4216'],
      'server_name':'example.com:{}'.format(backend_port),
      'domain_name':'https://example.com:{}'.format(backend_port),
      'flask_env':'development',
      'frontend_angular_app_url':'https://example.com:4216',
...
class PreviewConfigs(DevConfigs):
  ...
  def __init__(self) -> None:
    ...
    self.app['access_control_allow_origin'] = ["https://ui.preview.default.com"]
    self.app.pop('server_name')
    self.app.pop('domain_name')
    self.app['frontend_angular_app_url'] = "https://ui.preview.default.com"
    self.app['frontend_angular_app_domain'] = "ui.preview.default.com"
...
class ProdConfigs(DevConfigs):
  ...
  def __init__(self) -> None:
    super().__init__()
    self.app['flask_env'] = 'production'
    self.app['access_control_allow_origin'] = ["https://default.com"]
    self.app.pop('server_name')
    self.app.pop('domain_name')
    self.app['frontend_angular_app_url'] = "https://default.com"
    self.app['frontend_angular_app_domain'] = "www.default.com"
